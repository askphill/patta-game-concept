...
export async function subscribeToKlaviyo(email, { firstName, username, score, country } = {}) {
  ...
  const properties = {};
  if (username) properties.patta_game_username = username;
  if (typeof score === 'number') properties.patta_game_score = score;

  await Promise.all([
    klaviyoSubscribe(apiKey, listId, email),
    klaviyoProfileImport(apiKey, email, country, properties, firstName),
  ]);
}
